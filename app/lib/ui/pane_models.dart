...
/// One decoded radar image, ready for a pane to draw.
///
/// A [RadarFrame] from the engine paired with the image and bounds the map
/// needs. [image] and [bounds] are mutable because viewport sharpening
/// replaces the full-disk render with one cut to the visible box: same
/// frame, same source bytes, sharper picture over a smaller area.
///
/// Public, and outside the widget layer, so a pane's state can be owned by
/// [PaneController] rather than by the widget that draws it.
class DisplayFrame {
