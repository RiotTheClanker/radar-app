...
const _site = AddonSite(
  'XOKC', 'Club', 'OK', 35.4, -97.6, 1200, false,
  addonId: 'club',
);
...
void main() {
  group('templates', () {
    ...
    test('a short id can be given where it is not the last three letters', () {
      const s = AddonSite('UNIV1', 'U', '', 1, 2, 0, false,
          addonId: 'u', shortIdOverride: 'UV1');
  ...
  group('index listings', () {
    ...
    test('JSON of names and objects', () {
      ...
      expect(e.map((x) => x.url),
          ['https://e.org/l3/OKC/N0B/a_20260101_000000', 'https://e.org/l3/OKC/N0B/b']);
...
<a href="notes.txt">notes.txt</a>
      ...
      expect(e.map((x) => x.url.split('/').last),
          ['OKC_N0B_2026_01_01_00_00_00', 'OKC_N0B_2026_01_01_00_05_00']);
    });
    ...
    test('plain text, one per line, absolute URLs kept', () {
      ...
      expect(e.map((x) => x.url), ['https://cdn.e.org/f1', 'https://e.org/l3/OKC/N0B/f2']);
    });
  });

  group('listing a source', () {
    test('S3: walks back by day, pages, filters, and keeps the newest',
        () async {
      final asked = <String>[];
...
<Key>2026/07/27/XOKC/XOKC20260727_235500_V06</Key>
    ...
    test('replay drops files after the moment, and files with no time',
        () async {
    ...
    test("a source's headers never follow a file to another host", () async {
      ...
      await fetchSourceObject(src, 'https://data.e.org/XOKC/f1',
          client: client);
      await fetchSourceObject(src, 'https://cdn.other.org/f2',
          client: client);
  ...
  group('which source a site uses', () {
    ...
    test("an addon site's source is the one listed", () async {
      ...
      expect(keys.single, 'https://club.e.org/OKC/N0G/OKC_N0G_2026_01_01_00_00_00');
    });
  });
}
