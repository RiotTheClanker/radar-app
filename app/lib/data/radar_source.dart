...
///
/// A source can be a bucket, a web listing, or a folder on this device. What
/// the files are depends on the source's role: `level2` and `level3` sources
/// hold NEXRAD files, which the engine's own decoders read; an `open` source
/// holds files in the open radar format (docs/open-format.md), which is how
/// someone with their own parser brings data the app has no decoder for.
/// Nothing here decodes, the same as every other fetcher
...
enum SourceKind {
  ...
  /// A folder on this device: a radar receiver's output directory, a sync
  /// folder, files a converter writes out. [DataSource.url] is then the
  /// folder's path (a template, like a URL's), already resolved to an
  /// absolute one by the addon loader.
  folder,
}
...
/// (`N0B`, `N0G`, …) — or for an open-format source the app's short product
/// code (`REF`, `VEL`, `ZDR`, …) — `{tilt}` the elevation cut as numbered on
/// the toolbar (1-4), and `{yyyy}` `{MM}` `{dd}` the UTC date. A template with
...
class AddonSite extends NexradSite {
  ...
  /// Open-format files, for every product this site shows. When set it is
  /// used instead of [level2] and [level3]: the files carry their own
  /// product, units and colours, so which NEXRAD family a product belongs to
  /// no longer says anything about where its data is.
  final DataSource? open;

  bool get isOpen => open != null;
...
class NoSourceException implements Exception {
  ...
  @override
  String toString() => site.isOpen
      // An open-format site has every product it has, and no NEXRAD at all;
...
/// Recent open-format keys for [site]'s [product] (the app's short code:
/// `REF`, `VEL`, …) at [tilt], newest last.
Future<List<String>> listSiteOpen(
  AddonSite site,
  String product, {
  int? tilt,
  int count = 10,
...
/// One open-format file for [site].
Future<Uint8List> fetchSiteOpen(
  AddonSite site,
  String key, {
  http.Client? client,
}) =>
    fetchSourceObject(site.open!, key, client: client);
  ...
  try {
    ...
    for (var back = 0; back < days; back++) {
      ...
      final entries = switch (src.kind) {
        SourceKind.s3 => await _listS3(c, src, site, product, tilt, day),
        SourceKind.listing =>
          await _listIndex(c, src, site, product, tilt, day),
        SourceKind.folder => await _listFolder(src, site, product, tilt, day),
      };
...
Future<List<SourceEntry>> _listS3(
  ...
  final base =
      _trimSlash(expandTemplate(src.url, site: site, tilt: tilt, day: day));
  final prefix = expandTemplate(src.prefix,
      site: site, product: product, tilt: tilt, day: day);
...
Future<List<SourceEntry>> _listIndex(
  ...
  final url = expandTemplate(src.url,
      site: site, product: product, tilt: tilt, day: day);
  final resp = await c
...
/// The files in a local folder. A folder that does not exist (yet) is an
/// empty listing, not an error: a receiver that has not written today's
/// directory is a quiet radar, the same as an empty day in a bucket.
///
/// Time comes from the file name like every other source, falling back to
/// the file's modified time — a local file has one, where a URL does not, so
/// even undated names can be ordered and replayed.
Future<List<SourceEntry>> _listFolder(
  DataSource src,
  NexradSite site,
  String product,
  int? tilt,
  ...
  final path = expandTemplate(src.url,
      site: site, product: product, tilt: tilt, day: day);
  ...
  await for (final e in dir.list(followLinks: true)) {
    ...
    if (src.match != null && !src.match!.hasMatch(name)) continue;
    DateTime? time = entryTime(name);
    if (time == null) {
      try {
        time = (await e.lastModified()).toUtc();
      } catch (_) {}
    }
    out.add(SourceEntry(e.path, time));
  }
  return out;
}
...
Future<Uint8List> fetchSourceObject(
  ...
  if (src.kind == SourceKind.folder) {
    // The key is the path the listing found. Checked against the folder it
    // came from, so a key can only ever be a file this source listed.
    final file = File(url);
    if (!file.absolute.path.startsWith(_folderRoot(src))) {
      throw Exception('$url is not inside the source folder');
    }
...
/// The fixed part of a folder template — everything before the first
/// `{token}` — which every listed file sits under.
String _folderRoot(DataSource src) {
  final i = src.url.indexOf('{');
  return File(i < 0 ? src.url : src.url.substring(0, i)).absolute.path;
}
